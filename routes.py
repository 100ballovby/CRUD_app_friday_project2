...
def user(id):
    ...
    instance = None  # изначально сущность пользователя не существует
    for user in user_list:  # переюбираем все словари в списке с пользователями
        if user['id'] == id:  # если id совпал с каким-то юзером
            instance = user  # создаем сущность для передачи в шаблон
            print(instance)
